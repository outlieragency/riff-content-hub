...
export function OutlierCard({
  ...
  function onTrackCreator(e: React.MouseEvent) {
    e.preventDefault()
    e.stopPropagation()
    if (!video.channel_handle) return
    start(async () => {
      const res = await trackCreator(video.channel_handle as string)
      if (res.ok) setTrackState('done')
    })
  }
  ...
  return (
    <article className="group flex flex-col rounded-[14px] bg-card border border-border-soft overflow-hidden hover:shadow-lg transition-all">
      ...
      >
        ...
        {/* Top-right action — Save for tracked videos, Track-creator for shared */}
        {shared ? (
          <button
            onClick={onTrackCreator}
            disabled={pending || trackState === 'done' || !video.channel_handle}
            className={`absolute top-2 right-2 inline-flex items-center gap-1 px-2.5 h-8 rounded-full text-[11px] font-medium transition-all disabled:opacity-70 ${
              trackState === 'done'
                ? 'bg-green-600 text-white opacity-100'
                : 'bg-black/70 text-white opacity-0 group-hover:opacity-100 hover:bg-brand'
            }`}
            title={trackState === 'done' ? 'tracked' : 'track this creator'}
          >
            {pending ? (
            ...
            <span>{trackState === 'done' ? 'tracked' : 'track'}</span>
          </button>
        ) : (
          <button
            onClick={onToggle}
        ...
        {/* Top-left badge — Discover marker for shared, outlier flame otherwise */}
        {shared ? (
          <span className="absolute top-2 left-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-600/90 text-white text-[10px] font-semibold tracking-wide uppercase">
            <Sparkles size={10} />
            Discover
          </span>
        ) : (
          typeof video.outlier_score === 'number' &&
